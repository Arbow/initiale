...
%% than AVL trees.
%%
%% Add a new funtion to find ther nearest Entry of key
%% ---------------------------------------------------------------------
%% Operations:
%%
...
-export([empty/0, is_empty/1, size/1, lookup/2, get/2, insert/3,
	 ...
	 iterator/1, next/1, lookup_nearest/2]).
      
-export([test/0]).  %only use to test
...
%% key; if no such entry exists, return the smallest entry; if the tree is
%% empty, return {nil, nil}

%Tree is empty, return {nil, nil}
lookup_nearest(_SearchKey, {_, nil}) ->
    {nil, nil};
%lookup the nearest Entry
lookup_nearest(SearchKey, T={_, Tree}) ->
    case lookup_nearest1(SearchKey, Tree) of
        need_backtracking -> smallest(T);   %SearchKey larger than any EntryKey, return smallest Entry
        {found, Entry} -> Entry
    end.

%% Find specified key
lookup_nearest1(SearchKey, {SearchKey, Value, _, _}) ->
    {found, {SearchKey, Value}};
%% SearchKey < EntryKey && Entry.left is nil, return Entry
lookup_nearest1(SearchKey, {Key, Value, nil, _}) when SearchKey < Key ->
    {found, {Key, Value}};
%% SearchKey < EntryKey && Entry.left not nil, recursion Entry.left
lookup_nearest1(SearchKey, {Key, Value, Smaller, _}) when SearchKey < Key ->
    %io:format("~p < ~p, lookup left child ~p~n", [SearchKey, Key, Smaller]),
    case lookup_nearest1(SearchKey, Smaller) of
        need_backtracking -> {found, {Key, Value}}; %Entry's left children < Key, so select Entry
        Result -> Result
    end;
%% SearchKey > EntryKey && Entry.right is nil, backtracking until Parent==nil or Entry<>Parent.right
lookup_nearest1(SearchKey, {Key, _, _, nil}) when SearchKey > Key ->
    %io:format("~p > ~p, backtracking~n", [SearchKey, Key]),
    need_backtracking;
%% SearchKey > EntryKey && Entry.right not nil, recursion Entry.right
lookup_nearest1(SearchKey, {Key, _, _, Bigger}) when SearchKey > Key ->
    lookup_nearest1(SearchKey, Bigger).
...
test() ->
    T = lists:foldl(fun(V,T) -> treemap:insert(V,null,T) end, treemap:empty(), lists:seq(10, 50, 2)),
    io:format("Tree:~n~p~n", [T]),
    lists:map(fun(Key) -> io:format("~p: ~p~n",[Key,treemap:lookup_nearest(Key, T)]) end, lists:seq(0,60)).
